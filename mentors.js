...
async function getMentors() {
    return new Promise((resolve, reject) => {
        try {
            ...
            const index = store.index('isMentor');
...
function updateFiltersOptions(mentors) {
    if (fieldFilter) {
        const selectedField = fieldFilter.value;
        const uniqueFields = [...new Set(mentors.map((m) => m.field).filter(Boolean))].sort((a, b) => a.localeCompare(b));
        const options = ['<option value="all">All Disciplines</option>', ...uniqueFields.map((field) => `<option value="${field}">${field}</option>`)];
        fieldFilter.innerHTML = options.join('');
        if (selectedField !== 'all' && uniqueFields.includes(selectedField)) {
            fieldFilter.value = selectedField;
        }
    }

    if (yearFilter) {
        const selectedYear = yearFilter.value;
        const uniqueYears = [...new Set(mentors.map((m) => m.gradYear).filter(Boolean))].sort((a, b) => b - a);
        const options = ['<option value="all">All Graduations</option>', ...uniqueYears.map((year) => `<option value="${year}">${year}</option>`)];
...
function renderMentorCards(mentors) {
    ...
    mentors.forEach((mentor) => {
        ...
        card.className = 'bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6 flex flex-col gap-5 hover:border-purple-500/40 transition-colors';
        const initials = (mentor.name || '?').charAt(0).toUpperCase();
        const updated = mentor.lastUpdate ? new Date(mentor.lastUpdate).toLocaleString() : 'Unknown';
        card.innerHTML = `
            <div class="flex items-start justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-semibold">${mentor.name || 'Unnamed Mentor'}</h3>
                        <p class="text-slate-400 text-sm">${mentor.field || 'Discipline not specified'} • ${mentor.company || 'Company not provided'}</p>
                    </div>
                </div>
                <span class="px-3 py-1 rounded-full text-xs font-medium border bg-purple-500/10 text-purple-300 border-purple-500/30">Active Mentor</span>
            </div>
            <div class="grid sm:grid-cols-2 gap-4 text-sm">
                <div>
                    <p class="text-slate-400 uppercase text-xs mb-1">Graduation Year</p>
                    <p class="text-white font-medium">${mentor.gradYear || '—'}</p>
                </div>
                <div>
                    <p class="text-slate-400 uppercase text-xs mb-1">Email</p>
                    <p class="text-white font-medium break-all">${mentor.contactEmail || 'No email provided'}</p>
                </div>
                <div>
                    <p class="text-slate-400 uppercase text-xs mb-1">Phone</p>
                    <p class="text-white font-medium">${mentor.mobileNumber || 'No phone provided'}</p>
                </div>
                <div>
                    <p class="text-slate-400 uppercase text-xs mb-1">Last Updated</p>
                    <p class="text-white font-medium">${updated}</p>
                </div>
            </div>
            ${mentor.linkedinProfile ? `
                <a href="${mentor.linkedinProfile}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-sm text-blue-400 hover:text-blue-300 transition-colors">
...
function applyFilters() {
    if (!Array.isArray(allMentors)) return;
    let filtered = [...allMentors];

    const term = searchInput ? searchInput.value.trim().toLowerCase() : '';
    if (term) {
        filtered = filtered.filter((mentor) => {
            const haystack = [mentor.name, mentor.field, mentor.company, mentor.contactEmail]
                .filter(Boolean)
                .join(' ')
                .toLowerCase();
    ...
    if (fieldFilter && fieldFilter.value !== 'all') {
        filtered = filtered.filter((mentor) => (mentor.field || '').toLowerCase() === fieldFilter.value.toLowerCase());
    }

    if (yearFilter && yearFilter.value !== 'all') {
        const yearNumber = Number(yearFilter.value);
        filtered = filtered.filter((mentor) => Number(mentor.gradYear) === yearNumber);
    }
...
function openModal() {
    if (!alumniModal || !modalContent) {
        console.error('Mentor modal elements are missing from the DOM.');
        return;
    }
...
function setupModal() {
    ...
    if (addAlumniButtons.length === 0) {
        console.warn("No 'Add Mentor' triggers found on the mentor page.");
    } else {
    ...
    if (newAlumniForm) {
        newAlumniForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (modalMessage) {
                    modalMessage.textContent = 'Mentor profile created!';
                ...
                submitAlumniBtn.textContent = 'Create Mentor Profile';
            }
        });
    }
}

function updateMetrics(mentors) {
    const now = new Date();
    if (metrics.total) {
        metrics.total.textContent = mentors.length;
    }
    if (metrics.recent) {
        const recentCount = mentors.filter((m) => m.lastUpdate && now - m.lastUpdate <= 7 * 24 * 60 * 60 * 1000).length;
        metrics.recent.textContent = recentCount;
    }
    if (metrics.syncTime) {
        metrics.syncTime.textContent = now.toLocaleTimeString();
    }
}
...
async function loadMentors() {
    try {
        ...
        updateFiltersOptions(allMentors);
        applyFilters();
        updateMetrics(allMentors);
    } catch (error) {
        console.error('Failed to load mentor records:', error);
    }
}

async function init() {
    mentorsContainer = document.getElementById('mentors-list');
    emptyResults = document.getElementById('empty-results');
    searchInput = document.getElementById('search-mentor');
    fieldFilter = document.getElementById('filter-field');
    yearFilter = document.getElementById('filter-year');
    clearFiltersBtn = document.getElementById('clear-filters');
    addAlumniButtons = [document.getElementById('open-modal-nav')];

    metrics.total = document.getElementById('metric-total-mentors');
    metrics.recent = document.getElementById('metric-recent');
    ...
    try {
        ...
        setInterval(loadMentors, 10000);
    } catch (error) {
        console.error('Failed to initialize mentor page:', error);
    }
}
